...
export const SyndicsScreen: React.FC = () => {
  ...
  const handlePayResident = (resident: Resident) => {
    const params = new URLSearchParams({
      purpose: 'syndic_subscription',
      amount: '8500',
      description: `Cotisation villa ${resident.house_number}`,
      residentName: resident.name,
    });
    navigate(`/orders/payment?${params.toString()}`);
  };
      ...
      <div className="bg-brand-primary text-white px-6 pt-12 pb-8 rounded-b-[40px] shadow-lg">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-brand-on-primary/80 text-sm font-medium">Gestion de Concorde Shop</p>
          </div>
                    ...
                    <div className="flex gap-2">
                      {resident.subscription_status !== 'paid' && (
                        <>
                          ...
                          <button 
                            onClick={() => handlePayResident(resident)}
                            className="flex-1 bg-blue-50 text-blue-700 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 active:scale-95 transition-all"
                          >
                            <CreditCard size={14} />
                            Payer en ligne
                          </button>
