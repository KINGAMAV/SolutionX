...
import { Search, Construction, Droplets, Bolt, Paintbrush, Star, Verified, Zap, Bell, Loader2 } from 'lucide-react';
...
export const ArtisansScreen: React.FC = () => {
  ...
  const fetchArtisans = async () => {
    try {
      ...
      if (data) {
        const mappedArtisans: Artisan[] = data.map(a => ({
          id: a.id,
          name: a.name,
          category: a.category,
        ...
        <section>
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-brand-on-surface">
              {searchQuery ? `Résultats pour "${searchQuery}"` : 'Artisans recommandés'}
            </h3>
            <button className="text-brand-primary font-bold text-sm" onClick={() => setSearchQuery('')}>Voir tout</button>
          </div>
          ...
          <div className="space-y-4">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-10 gap-3 opacity-60">
                ...
                <span className="text-sm font-bold uppercase tracking-wider">Chargement des artisans...</span>
              </div>
              ...
              filteredArtisans.map((artisan) => (
                ...
                >
                  <div className="w-24 h-24 rounded-2xl overflow-hidden shrink-0 shadow-inner bg-brand-surface-low text-brand-outline flex items-center justify-center">
                    {artisan.avatar ? (
                  ...
                  <div className="flex flex-col justify-between flex-1 py-1">
                    <div>
                      <div className="flex justify-between items-start">
                        <h4 className="font-bold text-brand-on-surface line-clamp-1">{artisan.name}</h4>
                        <div className="flex items-center gap-1 bg-brand-secondary-fixed px-2 py-0.5 rounded-full shrink-0">
                    ...
                    <div className="flex items-center justify-between">
                      {artisan.verified && (
                        <div className="flex items-center gap-1.5 px-3 py-1 bg-brand-primary/5 rounded-full">
                          ...
                          <span className="text-[10px] font-bold text-brand-primary uppercase tracking-wide">Certifié</span>
                        </div>
                      )}
                      
                      <div className="text-[10px] font-black text-brand-on-surface-variant uppercase tracking-widest">
                        {artisan.hourlyRate} FCFA / h
                      </div>
                    </div>
                  </div>
                </motion.button>
              ...
              <div className="text-center py-10">
                <p className="text-brand-outline font-medium">Aucun artisan trouvé.</p>
              </div>
            )}
          </div>
        </section>
